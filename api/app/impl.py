...
def coupang_products(query, page='1', sort='scoreDesc'):
...
def gmarket_products(query, page='1', sort='7'):
...
def elevenStreet_products(query, page='1'):
...
def interpark_products(query, page=1):
...
def weMakePrice_products(query, page='1'):
...
def tmon_products(query, page='1'):
    url = TMON_SEARCH_URL
...
def all_products(query, page='1'):
    with ProcessPoolExecutor() as pool:
        futures = [
            pool.submit(coupang_products, query, page),
            pool.submit(gmarket_products, query, page),
            pool.submit(elevenStreet_products, query, page),
            pool.submit(interpark_products,query, page),
            pool.submit(auction_products,query, page),
            pool.submit(weMakePrice_products,query,page),
            pool.submit(tmon_products,query, page)
        ]
        result = {'result':list(map(lambda x:x.result(),futures))}
